...
import loader.elbrus.xml.XMLGaps;

/**
 * ActiveMQGaps
 * Получение информации от Эльбрус информации об окнах
...
public class ActiveMQGaps {
  ...
  public ActiveMQGaps(LAConfig laConfig, LBData lbData) {
  ...
  public boolean sendMessage(MRequest mRequest, int codeRoad, int requestIntervalTimeMinute,
    ...
    consumer.setMessageListener(new MessageListener() {
      ...
      public void onMessage(Message bytesMessage) {
        try {
          ...
          TimetableGapsMessage timetableGapsMessage = TimetableGapsMessage.parseFrom(inputStream);

          // Формируем XML документ
          XMLGaps xmlGaps = new XMLGaps();
          Document document = xmlGaps.createXMLFile(timetableGapsMessage, mRequest, dateStart, dateFinish, url);
    ...
    bytesMessage.setStringProperty("emq:message_operation", "get_timetable_gaps");
