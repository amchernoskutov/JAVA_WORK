...
import loader.elbrus.xml.XMLLimits;

/**
 * ActiveMQLimits
 * Получение информации от Эльбрус информации об ограничениях
...
public class ActiveMQLimits {
  ...
  public ActiveMQLimits(LAConfig laConfig, LBData lbData) {
  ...
  public boolean sendMessage(MRequest mRequest, int codeRoad, int requestIntervalTimeMinute,
    ...
    consumer.setMessageListener(new MessageListener() {
      ...
      public void onMessage(Message bytesMessage) {
        try {
          ...
          TimetableSpeedLimitsMessage timetableSpeedLimitsMessage = TimetableSpeedLimitsMessage.parseFrom(inputStream);

          // Формируем XML документ
          XMLLimits xmlLimits = new XMLLimits();
          Document document = xmlLimits.createXMLFile(timetableSpeedLimitsMessage, mRequest, dateStart, dateFinish, url);
    ...
    bytesMessage.setStringProperty("emq:message_operation", "get_timetable_speed_limits");
