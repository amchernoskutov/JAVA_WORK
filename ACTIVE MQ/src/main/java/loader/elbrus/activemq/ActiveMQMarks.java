...
import loader.elbrus.xml.XMLMarks;

/**
 * ActiveMQMarks
 * Получение информации от Эльбрус информации о пометках
...
public class ActiveMQMarks {
  ...
  public ActiveMQMarks(LAConfig laConfig, LBData lbData) {
  ...
  public boolean sendMessage(MRequest mRequest, int codeRoad, int requestIntervalTimeMinute,
    ...
    consumer.setMessageListener(new MessageListener() {
      ...
      public void onMessage(Message bytesMessage) {
        try {
          ...
          TimetableMarksMessage timetableMarksMessage = TimetableMarksMessage.parseFrom(inputStream);

          // Формируем XML документ
          XMLMarks xmlMarks = new XMLMarks();
          Document document = xmlMarks.createXMLFile(timetableMarksMessage, mRequest, dateStart, dateFinish, url);
    ...
    bytesMessage.setStringProperty("emq:message_operation", "get_timetable_marks");
