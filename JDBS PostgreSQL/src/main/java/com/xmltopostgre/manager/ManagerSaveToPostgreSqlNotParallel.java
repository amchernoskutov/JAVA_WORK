...
public class ManagerSaveToPostgreSqlNotParallel {
...
//public static final String PASSWORD = "psw:7-45-64adm";
  ...
  public ManagerSaveToPostgreSqlNotParallel() {
    //
  }

  private void writeToMarshrut(Integer id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    try {
  ...
  private void writeToSection(Integer m_id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    final int[] count = {1};
  ...
  private void writeToToplivo(Integer m_id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    final int[] count = {1};
  ...
  private void writeToPoezdka(Integer m_id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    final int[] count = {1};
  ...
  private void writeToSostav(Integer m_id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    final int[] count = {1};
  ...
  private void writeToSled(Integer m_id, Marshrut marshrut, JdbcTemplate jdbcTemplate) {
    final int[] count = {1};
  ...
  public void writeToPostgerSqlNotParallel(DataMarshrut dataMarshrut) {
    ...
    dataMarshrut.getMarshruts().forEach(marshrut -> {
      ...
      writeToMarshrut(count[0], marshrut, jdbcTemplate);
      if (marshrut.getM_section() != null) writeToSection(count[0], marshrut, jdbcTemplate);
      if (marshrut.getM_toplivo() != null) writeToToplivo(count[0], marshrut, jdbcTemplate);
      if (marshrut.getM_poezdka() != null) writeToPoezdka(count[0], marshrut, jdbcTemplate);
      if (marshrut.getM_sostav() != null)  writeToSostav(count[0], marshrut, jdbcTemplate);
      if (marshrut.getM_sled() != null) writeToSled(count[0], marshrut, jdbcTemplate);
    ...
    System.out.println("Начало записи в базу postgresql: " + startDate.getTime());
