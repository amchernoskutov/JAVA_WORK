...
/**
 * Loader SVLTR 
 * Loader SVLTR - REST Crient приложение, взаимодействует с Web Service СВЛ ТР.  
 * 
 * Приложение запускается из Main Scheduler-а по заданному расписанию и формирует
 * REST запрос или запросы к СВЛ ТР за определенный период времени.
 *  
 * Полученные ответы записывается в файл формата JSON, далее Loader направляет 
 * MQ Dispacher-у MQ сообщение о получении данных.   
 * 
 * Приложение использует конфигурационный файл configloadersvltr.xml для считывания 
 * параметров запроса.
 * 
