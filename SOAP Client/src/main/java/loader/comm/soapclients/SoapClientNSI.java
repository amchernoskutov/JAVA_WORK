...
public class SoapClientNSI {
  ...
  public SoapClientNSI(LSConfig lsConfig, LSData lsData, Jaxb2Marshaller jaxb2Marshaller) {
   ...
   */
  public boolean generateSOAPData (MRequest mRequest, int codeRoad, Date dataStartTime, int codeDepo, int requestIntervalTimeMinute, int requestId) {
    String message = "ID request-" + mRequest.getId() + ";Name request-" + mRequest.getName() + 
        ";Dor number-" + codeRoad + ";Depo number-" + codeDepo;
    String messageTemporary;
    MainForm mainForm = new MainForm();
    mainForm.setDateCreate(new Date());
    mainForm.setName(mRequest.getName());
    mainForm.setDor(Integer.toString(codeRoad));
    mainForm.setDepo(Integer.toString(codeDepo));
    ...
    try {
      respond = createSOAPRequest(mRequest, codeRoad, dataStartTime, codeDepo, requestId);
  ...
  private InputStream createSOAPRequest(MRequest mRequest, int codeRoad, Date dataStartTime, int codeDepo, int requestId) throws Exception {
    Z63LASEREPORTGETNSI request = new Z63LASEREPORTGETNSI();
    if (codeRoad != 0) {
      request.setCDOR(codeRoad);
    }
    request.setCDEPO(codeDepo);
    request.setDREFRESH(GeneralData.FORMAT_DATE_SOAP.format(dataStartTime));
